...
#    besbrugsart_øko = besbrugsart[findall(==(16), besbrugsart.BRUGSART_ID), :]
    ...
    koord = CSV.read(joinpath(datadir, "giskoord.csv"), DataFrame, types = Dict(:LAT => Float64, :LON => Float64))
    # A couple of missing lon-lat values - these have XKOOR and YKOOR of 0
    dropmissing!(koord)

    return (; dyr, dyrtilbes, slagt, fund, beskart, bes_chr, besbrugsart, koord)
end
